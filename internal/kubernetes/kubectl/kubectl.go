...
func (k *Kubectl) Initialize(kubeconfig []byte) error {
	...
	unstructuredClient, err := client.New(clientConfig, client.Options{})
	if err != nil {
		return fmt.Errorf("creating unstructed client: %w", err)
	}
	k.unstructuredClient = unstructuredClient
...
func (k *Kubectl) GetCRDs(ctx context.Context, namespace string) ([]apiextensionsv1.CustomResourceDefinition, error) {
...
func (k *Kubectl) GetCRs(ctx context.Context, namespace string) ([]apiextensionsv1.CustomResourceDefinition, error) {
	u := &unstructured.Unstructured{}
	u.SetKind()
	crds, err := k.unstructuredClient.Get(ctx, name, &unstructured.Unstructured{})
	if err != nil {
		return nil, fmt.Errorf("listing CRDs: %w", err)
	}
